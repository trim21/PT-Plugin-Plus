<template>
  <div class="set-download-clients">
    <v-alert :value="true" type="info">{{
      $t("settings.downloadPaths.index.title")
    }}</v-alert>
    <v-card>
      <v-card-title>
        <v-autocomplete
          ...
          <template slot="item" slot-scope="data">
            ...
            <v-list-tile-action>
              <v-list-tile-action-text>{{
                data.item.allowCustomPath
                  ? data.item.type
                  : $t("settings.downloadPaths.index.notSupport")
              }}</v-list-tile-action-text>
            </v-list-tile-action>
          </template>
        </v-autocomplete>
      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td>
            <div v-for="(path, index) in props.item.paths" :key="index">
              {{ path }}
            </div>
          </td>
          <td>
    ...
    <v-dialog v-model="dialogRemoveConfirm" width="300">
      <v-card>
        <v-card-title class="headline red lighten-2">{{
          $t("settings.downloadPaths.index.removeConfirmTitle")
        }}</v-card-title>

        <v-card-text>{{
          $t("settings.downloadPaths.index.removeConfirm")
        }}</v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn flat color="info" @click="dialogRemoveConfirm = false">
            ...
            <span class="ml-1">{{
              $t("settings.downloadPaths.index.cancel")
            }}</span>
          </v-btn>
          <v-btn color="error" flat @click="remove">
            <v-icon>check_circle_outline</v-icon>
            <span class="ml-1">{{
              $t("settings.downloadPaths.index.ok")
            }}</span>
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-snackbar v-model="itemDuplicate" top :timeout="3000" color="error">{{
      $t("settings.downloadPaths.index.itemDuplicate")
    }}</v-snackbar>
  </div>
</template>
